...
Exclude files or folders after a positive pattern match:
...
`watch.exclude` accepts a single value or an array of glob strings or regex objects. The aliases `watch.excludePattern`, `watch.excludePatterns`, `exclude`, and `excludePatterns` are also accepted for compatibility.
